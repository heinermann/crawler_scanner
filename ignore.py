"""Functions to determine which URLs to ignore"""
import re
from urllib.parse import urlparse


BANNED_URLS = set()
with open("banned_urls.txt", "r", encoding="utf-8") as banned:
    BANNED_URLS = set(line.strip() for line in banned)


BANNED_REGEXES = [re.compile(s, re.IGNORECASE) for s in [
    r".*_mp3\.zip$",
    r".*/(handouts|slides).zip\".*",
    r".*/albums?/.*",
    r".*/audio/.*",
    r".*/comics?/.*",
    r".*/docs?/.*",
    r".*/drivers?/.*",
    r".*/dvd\b.*/.*",
    r".*/(e|e-)?books?/.*",
    r".*/fonts?/.*",
    r".*/index.scm(\??.*)$",
    r".*/lessons?/.*",
    r".*/literature/.*",
    r".*/magazines?/.*",
    r".*/manuals?/.*",
    r".*/midi/.*",
    r".*/graphics/.*",
    r".*/mixtapes/.*",
    r".*/movies?/.*",
    r".*/mp3s?/.*",
    r".*/music/.*",
    r".*/sdk/.*",
    r".*/bible/.*",
    r".*/bibliography.*",
    r".*/patch(es)?/.*",
    r".*/pdf/.*",
    r".*/pdfs?/.*",
    r".*/photos?/.*",
    r".*/fotos?/.*",
    r".*/lectures?/.*",
    r".*/roms?/.*",
    r".*/rss/.*",
    r".*/vod/.*",
    r".*/spellchecker/.*",
    r".*/trainers?/.*",
    r".*/(utils?|utilities)/.*",
    r".*/screensavers?/.*",
    r".*/setup/.*",
    r".*/newspapers?/.*",
    r".*/memoires?/.*",
    r".*/presskits?/.*",
    r".*/styles?/.*",
    r".*/skins?/.*",
    r".*/financials?/.*",
    r".*/songs?/.*",
    r".*/musicas?/.*",
    r".*/musicals?/.*",
    r".*/sounds?/.*",
    r".*/temp/IndianJ\w+.*",
    r".*/temp/SaudiJ\w+.*",
    r".*/themes?/.*",
    r".*/videos?/.*",
    r".*/disclaimers?/.*",
    r".*\bgimp/.*",
    r".*wallpaper.*",
    r".*winamp.*",
    r".*payment\.php.*",
    r".*Encycopedia\.Britannica.*",
    r".*\.([a-z_]{3}|x86|x64|d64|3gp|mp3|mp4|m4v|wdgt|flst|jpeg|mpeg|com_|ppsx|docx|divx|html|aiff|cpp_|xvid)\.(zip|rar)$",
    r".*(%20|[._\-+/])(ebook|xvid|mp3|mp4|album|hdtv|480[pi]|720[pi]|1080[pi]|x264|dvdrip|bluray|mixtape|(%5b|%28)dis[ck]\+[1234]|cd[1234]|dvd[1-9]?|dvdscr|emudvd|win32|setup|osx|ipod|winexe|part[2-9][0-9]|xbox360|soundtrack)[._\-+%].*",
    r".*s\d\de\d\d[._\-+%].*",
    r".*_(win|osx|mac|src|exe|cs2|png|pps|dos|doc|fsx|jpg|x64|x86|php|css|img|wmv|pdf|vbs|psd|tif|dvd|gif|xml|xls|dwg|ttf|vlm|dxf|cad|com|linux|jar|pc|dll)\.(zip|rar)$",
]]

BANNED_DOMAIN_REGEXES = [re.compile(s, re.IGNORECASE) for s in [
    r".*\.ageofempires.*\..*",
    r".*\.blackberry.*\..*",
    r".*\.cheaters-heaven\.com$",
    r".*\.font.*",
    r".*\.gov(\.\w+)?$",
    r".*\.nokia\.com$",
    r".*\.codehaus\.org$",
    r".*\.sina\.com\.cn$",
    r"(.*\.|^)sourceforge\.net$",
    r".*\.state\.\w\w\.us$",
    r".*\.swipnet\.se$",
    r".*\.wincustomize\.com$",
    r".*\.deviantart\.net$",
    r".*fonts?\..*",
    r".*photoshop.*",
    r".*subtitles?\..*",
    r"^e?books\..*",
    r"^mp3\..*",
]]


def should_ignore_site(url_raw: str) -> bool:
    """Checks if a URL should be ignored or not."""

    url = urlparse(url_raw)
    netloc = url.netloc.lower().split(":")[0]
    if netloc in BANNED_URLS:
        return True

    for r in BANNED_DOMAIN_REGEXES:
        if r.match(netloc):
            return True

    for r in BANNED_REGEXES:
        if r.match(url_raw):
            return True

    return False
